    ...
    <div class="d-flex flex-col bg-slate-500">
        ...
        <div class="row">
            <input type="number" class="b-1 m-1 p-1 rounded-md" v-model="top_amount"
                @input="event => calculate(event.target.value)" @focus="topValue = true" />
        ...
        <div class="row">
            <input type="number" class="b-1 m-1 p-1 rounded-md" v-model="bottom_amount"
                @input="event => calculate(event.target.value)" @focus="topValue = false" />
...
import { type ICurrency, type ICurrencyRate } from '@/interfaces';

const top_currency = ref({ name: '', code: 'USD', rate: 1 })
const bottom_currency = ref({ name: '', code: 'USD', rate: 1 })
...
const currencies: Ref<ICurrency> = ref(await get_currencies())
const rates: Ref<ICurrencyRate> = ref(await get_rates())
